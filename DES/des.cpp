...
/*	char right[33];
	right[32]='\0';
	strcpy(right,sright.c_str());
//	cout<<right;
	
	int message32[32];
//	int message48[48];
	for(int k=0;k<32;k++)
		message32[k]=(int)(right[k]-48);
*/	
	for(int j=0;j<48;j++)
...
void encryption(char message[500],char key1[100])
{
	...
	/*while(flag<len)
	{
		temp[flag%8]=encryptionResult[flag];
		if((flag+1)%8==0)
		{			
...
void decryption(int secretMessage[500],char key2[100])
{
	...
	while(index<64)
	{
		...
		if((index+1)%64==0)//每次取出64位处理
		{
			...
			for(int i=15;i>=0;i--)
			{
				...
				//cout<<"one round decry result:"<<endl;
				//for(int j=0;j<64;j++)
				//	cout<<rearrangeMessage[j];
			}
	...
	//cout<<"decryptionResult"<<endl;
	//for(int k=0;k<len;k++)
	//		cout<<decryptionResult[k];
	//cout<<endl;
	int flag=0;
	int temp[8];
	char str[len];//解密后的明文 
